# Evaluating latent variable models of neural population activity: GPFA, VLMD, & multi-metric evaluation
...
- NOTE: The decoder itself is cross-validated Ridge regression, mapping the GPFA or VLMD latents to a behavioral variable — I ran it across several behaviors like whisking, pupil area, and respiration, swept across a range of latent dimensionalities, and scored with held-out R². Ridge specifically because the latent dimensions can be collinear, so plain OLS would be unstable. It's a much simpler decoder than what you'd actually deploy in a real-time BCI — the interesting part of my project was really the representation (GPFA vs. VLMD), not the decoder itself, which was intentionally kept simple so it wouldn't be doing extra work to make one latent model look better than the other.
