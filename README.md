...
- NOTE: The decoder itself is cross-validated Ridge regression, mapping the GPFA or VLMD latents to a behavioral variable — I ran it across several behaviors like whisking, pupil area, and respiration, swept across a range of latent dimensionalities, and scored with held-out R². It's a much simpler decoder than what you'd actually deploy in a real-time BCI — the interesting part of my project was really the representation (GPFA vs. VLMD), not the decoder itself, which was intentionally kept simple.
